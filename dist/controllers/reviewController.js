...
const getProductReviews = async (req, res) => {
    ...
    try {
        ...
        if (sort === "highest") {
            ...
            const totalReviews = await Review_model_1.default.countDocuments({ product: productId });
...
const getUserReviews = async (req, res) => {
    ...
    try {
        ...
        const reviews = await Review_model_1.default.find({ user: userId })
            ...
            .sort({ createdAt: -1 });
        const totalReviews = await Review_model_1.default.countDocuments({ user: userId });
        const totalPages = Math.ceil(totalReviews / limitNumber);
        const userStats = {
            totalReviews,
...
const getReviewStats = async (req, res) => {
    try {
        ...
        const allReviews = await Review_model_1.default.find();
        ...
        const ratingDistribution = {
            "5": await Review_model_1.default.countDocuments({ rating: 5 }),
            "4": await Review_model_1.default.countDocuments({ rating: 4 }),
            "3": await Review_model_1.default.countDocuments({ rating: 3 }),
            "2": await Review_model_1.default.countDocuments({ rating: 2 }),
            "1": await Review_model_1.default.countDocuments({ rating: 1 }),
        };
        // Number of products with reviews
        const productsWithReviews = await Product_model_1.default.countDocuments({
            reviewsCount: { $gt: 0 },
        });
        return res.status(200).json({
            error: false,
