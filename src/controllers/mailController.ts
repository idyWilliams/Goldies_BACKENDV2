...
const newsLetterSubscription = async (req: Request, res: Response) => {
    ...
    const transporter = nodemailer.createTransport({
      ...
      <div style="font-family: Arial, sans-serif; color: #333; line-height: 1.6; padding: 20px;">
        <div style="text-align: center;">
          <h2 style="color: #D4AF37;">🍰 Welcome to Goldies Newsletter! 🍰</h2>
        </div>
    ...
    const subscriberMailOptions = {
      ...
      to: email,
      subject: "🍰 Welcome to Goldies Newsletter!",
      text: "🍰 Welcome to Goldies Newsletter! 🍰",
    ...
    return res.status(200).json({ message: "Subscription email sent to both subscriber and company." });
  } catch (error) {
    console.error("Error sending email:", error);
    return res.status(500).json({ error: "Failed to send subscription email. Please try again later." });
  }
}
